...
M = M + object_m* I_Jp_E' *I_Jp_E ...
          + I_Jr_E'*R_IE*object_I*R_IE'*I_Jr_E;
fprintf('done!\n');

%% Compute gravity terms
fprintf('Computing gravity vector g... ');
g = g - object_m* I_Jp_E'* I_g_acc;
fprintf('done!\n');

%% Compute nonlinear terms
fprintf('Computing coriolis and centrifugal vector b and simplifying... ');
dJp_E = dAdt(I_Jp_E,q, dq);
dJr_E = dAdt(I_Jr_E,q, dq);
omega_E = I_Jr_E*dq;
I_sE = R_IE * object_I * R_IE';

b = b + I_Jp_E' * object_m * dJp_E * dq + ...
    I_Jr_E' * I_sE * dJr_E * dq + ...
    I_Jr_E' * cross( omega_E , I_sE * omega_E);
fprintf('done!\n');
